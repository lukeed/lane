...
export let commands: Command[] = [
	...
	{
		name: "note",
		usage: "lane note -p <path> [-a <anchor>] <text>",
	...
	},
	{
		...
		options: [
			{ flag: "--json", arg: null, about: "print one record per note as JSON instead of the table." },
		],
		example: "$ lane check\nfresh              7\nbody-drift         1\nsignature-changed  0\nanchor-missing     0\nunverifiable       0",
	},
	{
		name: "audit",
		usage: "lane audit [--base <ref>] [--review <mode>] [--json]",
		summary: "promotes pending notes, re-anchors across renames, reviews drift, ranks what is left, and evicts whatever is over budget.",
		options: [
			...
			{ flag: "--review", arg: "<mode>", about: "drift reviewer: auto, none, cmd or anthropic; default auto." },
			{ flag: "--review-cmd", arg: "<cmd>", about: "command that reads the drifted notes as JSON on stdin, e.g. 'claude -p'." },
			{ flag: "--review-max", arg: "<n>", about: "send at most this many drifted notes to the reviewer; default 20." },
			{ flag: "--json", arg: null, about: "print the outcome as JSON instead of the report." },
		],
		example: "$ lane audit\nmemory: +2 new; checked 8: 7 fresh, 1 body-drift,\n        0 signature-changed, 0 missing\n  review  src/sync.rs#fn reconnect",
	},
	{
		name: "done",
		usage: "lane done [--trunk <ref>] [--keep] [--squash] [--review <mode>]",
