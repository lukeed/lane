...
pub const SCENES: &[Scene] = &[
    Scene {
        ...
        steps: &[
            ...
            Do("ls -a"),
            Say("`.context/` is the memory. It is plain markdown at predictable paths, so an\n\
                 agent finds it without any tool integration. It is empty right now."),
            Do("find .context -type f | head"),
            Say("`AGENTS.md` is the part an agent always has in context. Three rules and a\n\
    ...
    Scene {
        ...
        steps: &[
            ...
            Do("printf 'pub fn parse(token: &str) -> Parsed {\\n    Parsed::from(token)\\n}\\n' >> src/auth.rs"),
            Do("git add -A && git commit -q -m 'add parse' && git log --oneline -1"),
            Say("Remember this. When you land work later, lane rebases onto whatever trunk\n\
    ...
    Scene {
        ...
        steps: &[
            ...
            Do("lane note -p src/auth.rs -a 'fn verify' 'must stay constant-time; an early return leaks token length'"),
            Say("`-a` is the anchor: what the note is *about*. `fn verify` binds it to that\n\
                 declaration, not to a line number, so the note survives the file being edited\n\
                 above it."),
            Say("The note is queued, not stored. It gets resolved and fingerprinted at the next\n\
    ...
    Scene {
        ...
        steps: &[
            ...
            Say("That is the habit worth building: `lane why <file>` before you change it.\n\
    ...
    Scene {
        ...
        steps: &[
            Say("A lane is a git worktree plus everything git ignores, cloned by reference\n\
                 where the filesystem supports it. Your caches come along without being copied."),
            Do("lane new fix-empty-token"),
            Say("It lives inside the repository at `.lanes/<name>`, excluded from git so it\n\
                 never shows up in `git status`."),
            Do("git status --porcelain && echo '(empty: the lane is invisible to git)'"),
            In(".lanes/fix-empty-token", "printf 'pub fn verify(token: &str) -> bool {\\n    !token.is_empty() && parse(token).is_valid()\\n}\\n' > src/auth.rs"),
            In(".lanes/fix-empty-token", "git add -A && git commit -q -m 'reject empty tokens' && git log --oneline -1"),
    ...
    Scene {
        ...
        steps: &[
            Say("You edited the body of `verify`. The note about it is still there, but the\n\
                 thing it describes has moved underneath it."),
            In(".lanes/fix-empty-token", "lane check"),
            Say("`body-drift` means the implementation changed while the signature held. Lane\n\
            ...
            Say("The `~` is the flag. Notes are not deleted for drifting; a human or a model\n\
    ...
    Scene {
        ...
        steps: &[
            ...
            Say("Now a `Why:` trailer in any commit message becomes a note. The form is\n\
                 `Why: <path>#<anchor> | <what must stay true>`."),
            In(".lanes/fix-empty-token", "printf 'pub fn verify(token: &str) -> bool {\\n    !token.is_empty() && parse(token).is_valid()\\n}\\n\\npub fn refresh(token: &str) -> String {\\n    rotate(token)\\n}\\n' > src/auth.rs"),
            In(".lanes/fix-empty-token", "git add -A && git commit -q -m 'add refresh\n\nWhy: src/auth.rs#fn refresh | callers depend on the rotated value, not the original'"),
            Say("Captured as you committed. No second command, no context switch — which is the\n\
    ...
    Scene {
        ...
        steps: &[
            Say("Open three more lanes. Each is a full checkout; on a copy-on-write filesystem\n\
                 they cost almost no disk."),
            Do("lane new agent-a && lane new agent-b && lane new agent-c"),
            Do("lane ls"),
            Say("Now have all three write a note about the same file, at the same anchor — the\n\
                 case that would deadlock a shared file."),
            In(".lanes/agent-a", "lane note -p src/auth.rs -a 'fn verify' 'agent-a: rejects empty tokens first'"),
            In(".lanes/agent-b", "lane note -p src/auth.rs -a 'fn verify' 'agent-b: parse is total, never panics'"),
            In(".lanes/agent-c", "lane note -p src/auth.rs -a 'fn verify' 'agent-c: called on every request, keep it allocation-free'"),
            Say("Nothing conflicts, because a note file is written once and never modified.\n\
    ...
    Scene {
        ...
        steps: &[
            ...
            Say("Rebased, memory folded in, trunk fast-forwarded, lane deleted. Trunk's history\n\
    ...
    Scene {
        ...
        steps: &[
            Say("Landing touches trunk, so it has to be exclusive. Lane holds a lock for the\n\
            ...
            Say("It refuses immediately and says why, rather than blocking or corrupting the\n\
    ...
    Scene {
        ...
        steps: &[
            Say("Lane will not silently overwrite something you edited. The skill file it\n\
            ...
            Say("It refuses and tells you how to proceed. The same rule covers commit hooks and\n\
