...
export let list: Act[] = [
	...
	},
	{
		...
		session: [
			...
			{ kind: 'gap', text: '' },
			{ kind: 'cmd', text: 'lane why src/auth.rs' },
			{ kind: 'gap', text: '' },
			{ kind: 'out', text: 'src/auth.rs#fn verify' },
			{ kind: 'warn', text: '  ~ early return leaks token length' },
			{ kind: 'warn', text: '                          [body-drift]' },
			{ kind: 'gone', text: '      01M0B9MBYB · fix-login · 2026-08-14' },
		],
	},
	{
		label: 'lane done',
